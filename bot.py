...
try: 
    ...
    if (len(feeds_alt) != 0 and random.randint(0, 333) == 0): 
        prefer_alt = True
    else: 
        prefer_alt = False
    ###################################### 
    ...
    #   SECRET  CODE S 2026. 3. 25 
    ###################################### 
    #evil = now + datetime.timedelta(minutes=-20)
    #if (len(feeds) == 0): 
    #    cs.execute('''
    #        delete from feeds 
    #        where date < ? 
    ...
    subprocess.run(["right/getfed.sh", f_url, f_title, f_lang])
    # 26. 3. 27 
...
if (has_trump): 
    subprocess.run(["composite", "-gravity", "SouthEast", "-geometry", "+0+0", "cf/trump.png", "bot_temp.png", "bot_temp.png"])
